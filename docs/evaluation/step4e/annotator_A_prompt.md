# STEP4E annotator A instructions

Annotate `step4e_annotation_packet_A.json` independently. Do not run AMATL; do not ask for or inspect predictions; and do not inspect the other annotation file. Return exactly one label per row, preserve `row_id`, do not alter dataset fields, and do not omit difficult rows—use `Unknown` where required.
