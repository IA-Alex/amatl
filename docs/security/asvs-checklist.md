...
| v5.0.0-13.4.1 browser process isolation | 2 | Pass for harness; core disabled | New process/filesystem namespaces, IPv4/IPv6 socket-deny seccomp, read-only runtime, private profile, cgroup memory/task/runtime limits and bounded DOM | `packaging/amatl-chromium-sandbox`; `docs/security/chromium-isolation.md` | Real Chromium JavaScript render plus loopback denial in dedicated workflow |
