...
| Deep → consumer | T/S | A hostile fragment is altered, detached from its source, or presented as authenticated truth | Evidence v2 binds exact UTF-8-safe byte ranges to source/extracted hashes, URL lineage, fetch/extractor metadata and deterministic IDs; fragments are capped at 8 × 512 bytes per document | `evidence.rs`; `tests/properties.rs`; `tests/deep_phase5.rs`; `docs/evidence-v2.md` | Hashes detect mismatch but do not prove publisher identity, factual truth or source authenticity; consumers must still render text as untrusted data |
